    ...
    VStack {
      if selectType == 0 { // 날짜별 보기
        ScrollView(showsIndicators: false) {
          ...
          // 상세 앨범 View로 이동
          NavigationLink("", destination: AlbumDetailView(viewModel: AlbumDetailListViewModel(albumId: selectAlbumId, kindType: nil), title: title, selectAlbumId: selectAlbumId), isActive: $showAlbumDetail)
        }
        ...
        ScrollView(showsIndicators: false) {
          ...
          // 상세 앨범 View로 이동
          NavigationLink("", destination: AlbumDetailView(viewModel: AlbumDetailListViewModel(albumId: nil, kindType: selectKindType), title: title, selectKindType: selectKindType), isActive: $showAlbumDetail)
        }
      }
    }
