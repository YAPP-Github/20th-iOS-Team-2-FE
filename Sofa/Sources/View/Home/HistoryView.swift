...
struct HistoryView: View {
  ...
  init(_ userId: Binding<Int>, _ isShowing: Binding<Bool>){
    _userId = userId
    _isShowing = isShowing
    historyViewModel.fetchHistory(userId: self.userId)
  }
...
//        print(page.index)
      ...
      ZStack(alignment: .center){
        if historyViewModel.isLoading{ // 값 받아오면
          VStack{
              ...
              VStack(alignment: .leading){
                HStack(alignment: .center){
                  ...
                  if historyViewModel.history.count > 0{
                    Text((String(historyViewModel.history[self.page.index == -1 ? 0 : self.page.index].createdDate).prefix(10)))
